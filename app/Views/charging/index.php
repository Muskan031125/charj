...
.hero-bg{background:linear-gradient(-45deg,#0f172a,#052e16,#0c1a2e,#14532d);background-size:400% 400%;animation:gradShift 12s ease infinite;}
...
.city-pill:hover{border-color:#86efac;color:#16a34a;}
.city-pill.active{background:linear-gradient(135deg,#16a34a,#0d9488);color:#fff;border-color:transparent;box-shadow:0 2px 8px rgba(22,163,74,.35);}
...
<!-- ═══ HERO ═══ -->
<section class="hero-bg relative overflow-hidden text-white pt-28 pb-16 px-4">
  <div class="absolute inset-0 opacity-10 pointer-events-none"
       style="background-image:radial-gradient(rgba(255,255,255,.5) 1px,transparent 1px);background-size:28px 28px"></div>
  <div class="absolute top-1/3 left-1/4 w-96 h-64 bg-green-500 opacity-5 blur-3xl rounded-full"></div>
  <div class="absolute bottom-0 right-1/4 w-72 h-72 bg-teal-400 opacity-5 blur-3xl rounded-full"></div>
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
      ...
      <div>
        ...
        <h1 class="text-4xl sm:text-5xl font-black leading-tight mb-3">
          EV Charging<br>Stations Near You
        </h1>
        <p class="text-slate-400 text-base max-w-lg">
          Real-time charging station data across India — fast DC chargers, AC wallboxes, and public charge points.
        </p>
        <!-- Stats -->
        <div class="flex flex-wrap gap-4 mt-6">
          <div class="flex items-center gap-2">
            <span class="text-2xl font-black text-green-400" x-text="allStations.length || '<?= count($stations ?? []) ?>'"></span>
            <span class="text-xs text-slate-400">stations found</span>
          </div>
          <div class="text-slate-600">·</div>
          <div class="text-xs text-slate-400" x-text="'Source: ' + dataSource"></div>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-4">
        <button @click="findNearMe()"
                :disabled="locating"
                class="flex items-center gap-4 rounded-2xl text-white font-bold px-7 py-5 text-base transition-all hover:-translate-y-0.5 hover:shadow-2xl hover:shadow-green-900/50 disabled:opacity-70"
                style="background:linear-gradient(135deg,#16a34a,#0d9488);box-shadow:0 8px 32px rgba(22,163,74,.3)">
          <div x-show="!locating" class="w-12 h-12 rounded-xl bg-white/20 flex items-center justify-center text-2xl flex-shrink-0">📍</div>
          <div x-show="locating" x-cloak class="w-12 h-12 rounded-xl bg-white/20 flex items-center justify-center flex-shrink-0">
          ...
          <div>
            <div class="font-black text-lg" x-text="locating ? 'Locating you...' : 'Find Chargers Near Me'"></div>
            <div class="text-green-100 text-xs font-medium" x-text="locating ? 'Getting your GPS...' : 'Uses your location · Works instantly'"></div>
          </div>
        </button>
        <p x-show="locErr" x-cloak x-text="locErr" class="text-red-300 text-xs text-center"></p>
        <!-- Quick cities -->
        <div class="flex flex-wrap gap-2">
          <?php foreach (['Delhi','Mumbai','Bangalore','Pune','Hyderabad','Chennai'] as $qc): ?>
          <button @click="loadCity('<?= strtolower($qc) ?>')"
                  class="text-xs font-semibold text-slate-400 hover:text-green-400 transition-colors px-3 py-1.5 rounded-xl hover:bg-white/10 border border-white/10 hover:border-green-500/40">
            <?= $qc ?>
          </button>
          <?php endforeach; ?>
        </div>

  ...
  <div class="max-w-7xl mx-auto px-4 py-10">
      ...
      <div class="bg-white rounded-3xl shadow-sm ring-1 ring-green-200 overflow-hidden">
        <div class="bg-gradient-to-r from-green-600 to-green-500 px-6 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-green-100 text-xs">Your future EV details</p>
          </div>
        </div>
          ...
          <div class="grid gap-4">
            <label class="grid gap-1">
              ...
              <input type="number" x-model="ev.price" min="50000" max="10000000" step="10000"
                @input="calc()"
                class="rounded-xl border border-slate-200 px-4 py-3 font-bold text-slate-800 focus:border-green-500 focus:outline-none focus:ring-1 focus:ring-green-500">
            </label>
            ...
            <div class="grid grid-cols-2 gap-3">
              <label class="grid gap-1">
                ...
                <select x-model="ev.tenure" @change="calc()" class="rounded-xl border border-slate-200 px-4 py-3 font-bold text-slate-800 focus:border-green-500 focus:outline-none">
            ...
            <div class="bg-green-50 rounded-2xl p-4 space-y-3">
              <h3 class="text-xs font-bold text-green-700 uppercase tracking-wide">Operating Costs</h3>
              <label class="grid gap-1">
                <span class="text-xs text-slate-600">Electricity Rate (₹/kWh)</span>
                <div class="flex items-center gap-3">
                  <input type="range" x-model="ev.elecRate" min="4" max="15" step="0.5" @input="calc()" class="flex-1 accent-green-500">
                  <span class="w-12 text-center font-bold text-green-700 bg-green-100 rounded-lg py-1 text-sm" x-text="'₹' + Number(ev.elecRate).toFixed(1)"></span>
                </div>
              </label>
              <label class="grid gap-1">
                <span class="text-xs text-slate-600">EV Efficiency (km/kWh)</span>
                <div class="flex items-center gap-3">
                  <input type="range" x-model="ev.efficiency" min="2" max="12" step="0.5" @input="calc()" class="flex-1 accent-green-500">
                  <span class="w-12 text-center font-bold text-green-700 bg-green-100 rounded-lg py-1 text-sm" x-text="Number(ev.efficiency).toFixed(1)"></span>
                </div>
              </label>
              <div class="grid grid-cols-2 gap-3">
                <label class="grid gap-1">
                  ...
                  <input type="number" x-model="ev.insurance" step="500" @input="calc()"
                    class="rounded-lg border border-green-200 px-3 py-2 text-sm font-bold focus:border-green-500 focus:outline-none">
                </label>
                <label class="grid gap-1">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      <div class="bg-green-600 rounded-2xl p-5 text-white">
        <div class="text-xs font-semibold text-green-100 uppercase tracking-wide mb-2">5-Year EV Cost</div>
      ...
      <div class="bg-[#0d2137] rounded-2xl p-5 text-white col-span-2">
        ...
        <div class="flex flex-wrap gap-4 mt-2">
          <div class="text-slate-300 text-xs">Monthly saving: <span class="text-white font-bold" x-text="fmt(r.monthlySaving)"></span></div>
          <div class="text-slate-300 text-xs">Break-even: <span class="text-white font-bold" x-text="r.breakEven + ' months'"></span></div>
          <div class="text-slate-300 text-xs">CO₂ saved: <span class="text-green-400 font-bold" x-text="r.co2Saved + ' tonnes/yr'"></span></div>
        </div>
      </div>
    </div>

...
<div x-data="subsidyCalc()" x-init="initRange()" class="pb-16" style="background:linear-gradient(180deg,#f0fdf4 0%,#f8fafc 280px)">
  ...
  <!-- ── HERO ── -->
  <div class="hero-sm relative overflow-hidden pt-28 pb-10 px-4" style="background:linear-gradient(135deg,#022C22 0%,#0A3D2B 60%,#0D4A2A 100%)">
    ...
    <div class="absolute top-0 right-0 w-96 h-72 rounded-full pointer-events-none" style="background:radial-gradient(circle,rgba(0,230,118,.12),transparent 65%);transform:translate(30%,-30%)"></div>
    <div class="absolute bottom-0 left-0 w-64 h-48 rounded-full pointer-events-none" style="background:radial-gradient(circle,rgba(0,168,150,.08),transparent 70%);transform:translate(-20%,20%)"></div>

    <div class="relative max-w-6xl mx-auto">
      <!-- Badge + Heading -->
      <div class="max-w-2xl">
        <div class="anim-fade-up inline-flex items-center gap-2 rounded-full border border-green-400/30 px-3 py-1 text-xs font-bold uppercase tracking-widest mb-4 text-green-300" style="background:rgba(0,230,118,.12)">
          <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
          Updated for 2024–25 Schemes
        </div>
        <h1 class="anim-fade-up delay-1 text-3xl sm:text-4xl lg:text-5xl font-black text-white leading-tight mb-3">Find Your EV Subsidy</h1>
        <p class="anim-fade-up delay-2 text-green-200 text-sm sm:text-base">Most buyers miss <strong class="text-white">₹15,000–₹1,50,000</strong> in government benefits. Check yours in 2 minutes — free.</p>
      </div>
      <!-- Stats row — always horizontal, flush left -->
      <div class="anim-fade-up delay-3 mt-8 grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-0 sm:divide-x sm:divide-green-700/40 max-w-xl">
        ...
        <div class="sm:px-5 first:pl-0">
          <div class="text-xl font-black text-white"><?= $s[0] ?></div>
          <div class="text-green-300/60 text-xs mt-0.5"><?= $s[1] ?></div>
        </div>
        <?php endforeach; ?>
      </div>
    </div>
          ...
          <div class="flex items-center gap-2 flex-1">
            <div class="flex items-center gap-1.5 min-w-0">
              <div :class="{'text-white step-active-pulse':step===i+1,'text-white':step>i+1,'bg-slate-200 text-slate-400':step<i+1}"
                :style="step>i+1?'background:#00C060':step===i+1?'background:#022C22;border:2px solid #00E676':''"
  ...
  <div class="max-w-6xl mx-auto px-4 py-5 sm:py-6">
      ...
      <div class="lg:grid lg:grid-cols-3 lg:gap-6 lg:items-start">
        ...
        <div class="lg:col-span-1 mt-3 lg:mt-0">
          ...
          <div x-show="selectedState" x-transition:enter="transition ease-out duration-250" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
               class="rounded-2xl p-4 relative overflow-hidden" style="background:linear-gradient(135deg,#022C22,#0A3D2B);border:1px solid rgba(0,230,118,.2);box-shadow:0 6px 24px rgba(0,230,118,.1)">
    ...
    <div x-show="step===3" x-cloak x-transition:enter="transition ease-out duration-250" x-transition:enter-start="opacity-0 translate-y-3" x-transition:enter-end="opacity-100 translate-y-0">
      ...
      <div class="lg:grid lg:grid-cols-2 lg:gap-5 mb-5">
              ...
              <template x-if="vehicleType==='2w'">
                <template x-for="p in [75000,100000,130000,160000,200000]" :key="p">
                  <button @click="vehiclePrice=p;updateRange($refs.rangeSlider)"
                    :style="vehiclePrice==p?'background:#022C22;color:#00E676;border-color:#022C22':''"
        ...
        <!-- Preview stats card -->
        <div class="rounded-2xl p-5 flex flex-col justify-center" style="background:linear-gradient(135deg,#022C22,#0A3D2B);border:1px solid rgba(0,230,118,.18);box-shadow:0 3px 16px rgba(0,0,0,.15)">
    ...
    <div x-show="step===4" x-cloak x-transition:enter="transition ease-out duration-250" x-transition:enter-start="opacity-0 translate-y-3" x-transition:enter-end="opacity-100 translate-y-0">
      ...
      <!-- Result banner -->
      <div class="rounded-2xl p-5 sm:p-6 mb-5 relative overflow-hidden" style="background:linear-gradient(135deg,#022C22 0%,#0A3D2B 60%,#0D5A2A 100%);box-shadow:0 8px 32px rgba(0,230,118,.15);border:1px solid rgba(0,230,118,.2)">
      ...
      <div class="lg:grid lg:grid-cols-3 lg:gap-5 mb-5">
        ...
        <div class="lg:col-span-1 flex flex-col gap-3">
          ...
          <!-- Lead form -->
          <div class="rounded-2xl p-5 flex-1 relative overflow-hidden" style="background:linear-gradient(135deg,#022C22,#0A3D2B);border:1px solid rgba(0,230,118,.18);box-shadow:0 6px 24px rgba(0,0,0,.15)">
            ...
            <div class="relative">
              ...
              <form class="space-y-2" @submit.prevent="submitLead">
                <input type="tel" x-model="lead.mobile" placeholder="Mobile number *" required pattern="[6-9]\d{9}" maxlength="10" inputmode="numeric"
                  ...
                  class="w-full rounded-lg px-3 py-2 text-white placeholder-slate-400 text-xs focus:outline-none transition-all"
                  style="background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.2)" onfocus="this.style.borderColor='#00E676'" onblur="this.style.borderColor='rgba(255,255,255,.2)'">

...
.range-btn.active,.price-btn.active{background:#00E676!important;color:#022C22!important;border-color:#00E676!important}
.peer:checked ~ .range-pill{background:#00E676!important;color:#022C22!important;border-color:#00E676!important}
...
<div
  ...
  x-init="init()"
  class="min-h-screen"
  style="background:#F5FFF7"
>

<!-- PAGE HERO -->
<div class="hero-sm relative overflow-hidden pt-28 pb-14 px-4" style="background:linear-gradient(160deg,#F0FFF4 0%,#E8FFEE 50%,#F0FFF4 100%)">
  <!-- Subtle mesh grid -->
  <div class="absolute inset-0 pointer-events-none" style="background-image:radial-gradient(rgba(0,230,118,.12) 1px,transparent 1px);background-size:28px 28px;opacity:.5"></div>
  <!-- Ambient green edge glows -->
  <div class="absolute top-0 right-0 w-96 h-72 pointer-events-none" style="background:radial-gradient(ellipse at 80% 20%,rgba(0,230,118,.18),transparent 65%);filter:blur(40px)"></div>
  <div class="absolute bottom-0 left-0 w-80 h-56 pointer-events-none" style="background:radial-gradient(ellipse at 20% 80%,rgba(105,255,151,.14),transparent 65%);filter:blur(40px)"></div>
  <!-- Center white bloom -->
  <div class="absolute inset-0 pointer-events-none" style="background:radial-gradient(ellipse at 50% 40%,rgba(255,255,255,.7) 0%,transparent 60%)"></div>

  <div class="relative max-w-7xl mx-auto">
    <nav class="text-xs mb-5 flex flex-wrap items-center gap-1.5 hero-in-1" aria-label="Breadcrumb">
      <a href="<?= base_url('/') ?>" class="text-slate-400 transition-colors" onmouseover="this.style.color='#374151'" onmouseout="this.style.color=''">Home</a>
    ...
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
      <div>
        <div class="inline-flex items-center gap-2 rounded-full px-3.5 py-1.5 text-xs font-black uppercase tracking-widest mb-4 hero-in-1"
             style="background:rgba(0,230,118,.1);border:1.5px solid rgba(0,230,118,.25);color:#00963C">
        ...
        <h1 class="text-3xl sm:text-4xl md:text-5xl font-black leading-tight tracking-tight hero-in-2" style="color:#0F172A">
          <?= esc($title ?? 'All EVs in India') ?>
        </h1>
        <p class="text-sm mt-2 font-medium max-w-lg hero-in-3" style="color:#6B7280">
          <?= esc($subtitle ?? 'Compare prices, range & features across all electric vehicles') ?>
        </p>
        <div class="flex flex-wrap gap-2 mt-6 hero-in-4">
          <div class="rounded-2xl px-5 py-3 text-center stat-pop-0" style="background:rgba(255,255,255,.85);border:1px solid rgba(0,230,118,.15);backdrop-filter:blur(8px);box-shadow:0 2px 12px rgba(0,0,0,.05)">
            <div class="text-2xl font-black text-slate-800"><?= !empty($savedMode) ? count($vehicles ?? []) : (number_format($totalVehicles ?? count($vehicles ?? [])) . '+') ?></div>
            <div class="text-[10px] uppercase tracking-widest font-bold mt-0.5" style="color:#94A3B8"><?= !empty($savedMode) ? 'Saved EVs' : 'EVs Listed' ?></div>
          </div>
          <div class="rounded-2xl px-5 py-3 text-center stat-pop-1" style="background:rgba(0,230,118,.08);border:1px solid rgba(0,230,118,.2);backdrop-filter:blur(8px)">
            <div class="text-2xl font-black" style="color:#00963C">₹59k</div>
            <div class="text-[10px] uppercase tracking-widest font-bold mt-0.5" style="color:#94A3B8">Starting From</div>
          </div>
          <div class="rounded-2xl px-5 py-3 text-center stat-pop-2" style="background:rgba(0,230,118,.08);border:1px solid rgba(0,230,118,.2);backdrop-filter:blur(8px)">
            <div class="text-2xl font-black" style="color:#00963C">700km+</div>
            <div class="text-[10px] uppercase tracking-widest font-bold mt-0.5" style="color:#94A3B8">Best Range</div>
          </div>
        </div>
      </div>
  ...
  <div class="lg:grid lg:grid-cols-[280px_1fr] lg:gap-8 flex flex-col gap-6">
    ...
    <div class="lg:hidden flex items-center justify-between gap-3 mb-1">
      <button
        ...
        style="background:#00E676;color:#022C22"
      >
        ...
        Filters
        <?php if ($hasFilters): ?>
        <span class="rounded-full w-5 h-5 text-xs flex items-center justify-center font-bold" style="background:#022C22;color:#00E676">
          <?= count($chips) ?>
        </span>
        <?php endif; ?>
      </button>
        ...
        <form method="GET" action="<?= base_url('vehicles') ?>" x-data="filterForm()" class="divide-y" style="--tw-divide-opacity:1;border-color:rgba(0,168,150,.06)">
          ...
          <div class="px-5 py-4 space-y-2" style="background:#F7FFFE">
            <button type="submit"
              ...
              style="background:#00E676;color:#022C22;box-shadow:0 4px 14px rgba(0,230,118,.3)"
              onmouseover="this.style.boxShadow='0 6px 20px rgba(0,230,118,.45)';this.style.transform='translateY(-1px)'"
    ...
    <main class="flex-1 min-w-0">
      ...
      <!-- Saved EVs banner -->
      <div class="flex items-center justify-between gap-3 mb-5 px-4 py-3 rounded-2xl"
      ...
      <div class="flex flex-wrap items-center gap-2 mb-5">
        ...
        <a href="<?= base_url('vehicles') . $removeUrl ?>"
           ...
           style="background:#FFFFFF;border:1px solid rgba(0,230,118,.22);color:#334155"
           onmouseover="this.style.color='#EF4444';this.style.borderColor='rgba(239,68,68,.3)';this.style.background='rgba(239,68,68,.03)'"
           onmouseout="this.style.color='#334155';this.style.borderColor='rgba(0,230,118,.22)';this.style.background='#FFFFFF'">
          <?= esc($chip['label']) ?> <span style="opacity:.5">✕</span>
        </a>
        <?php endforeach; ?>
      ...
      <?php else: ?>
      <!-- Empty state -->
      <div class="text-center py-20 rounded-2xl" style="background:#FFFFFF;border:1px solid rgba(0,168,150,.1)">
        <div class="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-6"
             style="background:rgba(0,168,150,.06);border:1px solid rgba(0,168,150,.15)">
          <svg class="w-10 h-10" style="color:#00A896;opacity:.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
          </svg>
        </div>
        <h3 class="text-2xl font-bold mb-2" style="color:#0F172A">No EVs Found</h3>
        <p class="mb-8 max-w-sm mx-auto text-sm leading-relaxed" style="color:#64748B">
        ...
        <div class="pt-6" style="border-top:1px solid rgba(0,168,150,.08)">
...
<div
  ...
  class="fixed bottom-0 left-0 right-0 z-40 text-white shadow-2xl"
  style="background:rgba(4,12,30,.97);border-top:2px solid #00A896;backdrop-filter:blur(20px)"
  role="region"
  aria-label="Compare bar"
>
  <div class="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4 flex-wrap">
    <div class="flex items-center gap-3 flex-wrap min-w-0">
      <svg class="w-5 h-5 fill-current hidden sm:block" style="color:#1AFFCC" viewBox="0 0 24 24" aria-hidden="true"><path d="M13 2L4.09 12.97H11L10 22L20.91 11.03H14L13 2Z"/></svg>
      ...
      <div class="flex items-center gap-2">
        <template x-for="(item, idx) in compareList" :key="item.id">
          <div class="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm" style="background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.2)">
        ...
        <template x-if="compareList.length < 3">
          <div class="flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs text-slate-400" style="border:1px dashed rgba(255,255,255,.2)">

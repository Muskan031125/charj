  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .nav-dropdown {
      ...
      box-shadow:0 4px 6px rgba(0,0,0,.04),0 20px 40px rgba(0,0,0,.08),0 0 0 1px rgba(0,230,118,.04);
    }
    .nav-link {
    ...
    .nav-link:hover { color:#00A896; background:rgba(0,230,118,.07); }
...
<header class="fixed inset-x-0 z-[200]"
        style="top:<?= session()->get('admin_previewing_as_customer') ? '40px' : '0' ?>;background:linear-gradient(160deg,#F8FBFF 0%,#EEF6FF 100%);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border-bottom:1px solid #E3EEFF;box-shadow:0 1px 0 rgba(0,0,0,.04),0 4px 20px rgba(0,0,0,.04)">
  <div class="mx-auto max-w-7xl px-4 sm:px-6">
    <div class="flex h-16 items-center justify-between gap-4">

      <!-- Logo -->
      <a href="<?= base_url() ?>" class="flex items-center gap-2.5 flex-shrink-0 group" aria-label="Charj.in Home">
        <div class="relative transition-transform duration-200 group-hover:scale-105">
          <svg viewBox="0 0 36 36" class="h-9 w-9" fill="none">
            <circle cx="18" cy="18" r="17" fill="rgba(0,230,118,.1)" stroke="rgba(0,230,118,.28)" stroke-width="1.5"/>
            <path d="M26 10.5A10 10 0 1 0 26 25.5" stroke="#00E676" stroke-width="3.2" stroke-linecap="round" fill="none"/>
            <path d="M19.5 9L13 19h5l-1.5 8L24.5 17H20L19.5 9Z" fill="#00E676"/>
          </svg>
          <span class="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full" style="background:#00E676;border:1.5px solid #fff;box-shadow:0 0 6px rgba(0,230,118,.6)"></span>
        </div>
        <div class="leading-none">
          <span class="font-black text-xl tracking-tight text-slate-800">charj</span><span class="font-black text-xl tracking-tight" style="color:#00C060">.in</span>
          <div class="text-[9px] font-semibold uppercase tracking-widest hidden sm:block" style="color:#94A3B8">India's EV Marketplace</div>
        </div>
      </a>

      <!-- Desktop Navigation -->
      <nav class="hidden md:flex items-center gap-0.5 flex-1 justify-center" aria-label="Main navigation">
        <a href="<?= base_url('explore') ?>" class="nav-link">Explore</a>
        <?php
        $navLinks = [
          ...
          ['label'=>'Tools','href'=>'#','sub'=>[
            ...
            ['label'=>'Trip Range Check',      'href'=>base_url('can-i-make-it'),   'svg'=>'M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7'],
            ['label'=>'Charger Compatibility', 'href'=>base_url('charger-check'),   'svg'=>'M17 8l4 4m0 0l-4 4m4-4H3'],
            ['label'=>'Resale Estimator',      'href'=>base_url('resale-estimator'),'svg'=>'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6'],
        ...
        <div class="relative" x-data="{open:false}" @mouseenter="open=true" @mouseleave="open=false">
          <button class="nav-link" :class="open && '!text-[#00A896] bg-[rgba(0,168,150,.07)]'">
            <?= esc($link['label']) ?>
            <svg class="h-3.5 w-3.5 opacity-50 transition-transform duration-200" :class="open?'rotate-180':''" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path d="M19 9l-7 7-7-7"/></svg>
          </button>
          <div x-show="open" x-cloak
               ...
               class="nav-dropdown absolute top-full left-1/2 -translate-x-1/2 mt-2 w-52 rounded-2xl p-1.5 z-50">
            <?php foreach ($link['sub'] as $sub): ?>
            <a href="<?= esc($sub['href']) ?>" class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-500 hover:bg-green-50 hover:text-[#00A896] transition-all duration-150 group">
              <span class="flex-shrink-0 w-7 h-7 rounded-lg flex items-center justify-center" style="background:rgba(0,168,150,.08);border:1px solid rgba(0,168,150,.12)">
              ...
              <svg class="w-3.5 h-3.5 ml-auto opacity-0 group-hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path d="M9 5l7 7-7 7"/></svg>
            </a>
            <?php endforeach; ?>
          </div>
        </div>
        <?php else: ?>
        <a href="<?= esc($link['href']) ?>" class="nav-link"><?= esc($link['label']) ?></a>
      ...
      <div class="flex items-center gap-2 flex-shrink-0">
        ...
        <button type="button"
           ...
           @click="if(count > 0){ var slugs=(window.charjGetWishlist()||[]).map(function(x){return x.slug}).join(','); window.location.href='<?= base_url('vehicles') ?>?saved='+encodeURIComponent(slugs); }"
        ...
        <a href="<?= base_url('find-my-ev') ?>" onclick="charjTrack('header_cta_click',{location:'header'})"
           class="hidden sm:inline-flex items-center gap-2 font-bold text-sm px-5 py-2.5 rounded-full transition-all duration-200 ripple-btn flex-shrink-0"
           style="background:linear-gradient(135deg,#00E676,#69FF97);color:#022C22;box-shadow:0 4px 14px rgba(0,230,118,.28)"
           onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 8px 20px rgba(0,230,118,.38)'"
           onmouseout="this.style.transform='';this.style.boxShadow='0 4px 14px rgba(0,230,118,.28)'">
...
<!-- FOOTER -->
<footer class="mt-16 sm:mt-24 pb-20 md:pb-0" style="background:linear-gradient(180deg,#EEFFF3 0%,#E6FFED 100%);color:#4B5563;border-top:1px solid rgba(0,230,118,.14)" role="contentinfo">
  <div class="mx-auto max-w-7xl px-4 sm:px-6 py-16">
    <div class="grid gap-10 md:grid-cols-2 lg:grid-cols-5">
      ...
      <div class="lg:col-span-2">
        <a href="<?= base_url() ?>" class="flex items-center gap-2.5 mb-5">
          ...
          <span class="font-black text-xl tracking-tight text-slate-800">charj<span style="color:#00C060">.in</span></span>
        </a>
        <p class="text-sm leading-relaxed text-slate-500 max-w-xs">India's dedicated EV marketplace. Discover, compare and choose the right electric vehicle — one platform, every brand.</p>
        ...
        <div class="mt-6 flex gap-2">
          ...
          <a href="#" aria-label="Charj.in on <?= $label ?>"
             class="w-9 h-9 flex items-center justify-center rounded-xl transition-all duration-200"
             style="background:rgba(255,255,255,.7);border:1px solid rgba(0,230,118,.18);color:#6B7280"
             onmouseover="this.style.background='#00E676';this.style.borderColor='#00E676';this.style.color='#022C22'"
             onmouseout="this.style.background='rgba(255,255,255,.7)';this.style.borderColor='rgba(0,230,118,.18)';this.style.color='#6B7280'">
        ...
        <div class="mt-5 flex flex-wrap gap-2">
          <?php foreach (['100% Free','FAME II Data','All Major Brands'] as $b): ?>
          <span class="flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium" style="background:rgba(255,255,255,.7);border:1px solid rgba(0,230,118,.2);color:#374151">
            <svg class="w-3 h-3 flex-shrink-0" style="color:#00C060" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
            <?= $b ?>
          </span>
          <?php endforeach; ?>
        </div>
      ...
      <!-- Explore EVs -->
      <div>
        <h3 class="mb-4 font-bold text-xs uppercase tracking-widest" style="color:#0F172A">Explore EVs</h3>
        <ul class="space-y-2.5 text-sm">
          <?php foreach ([['Electric Scooters','electric-scooters'],['Electric Cars','electric-cars'],['Electric Bikes','electric-bikes'],['E-Rickshaws','e-rickshaws'],['Commercial EV','commercial-ev'],['All EVs','vehicles'],['Compare EVs','compare']] as [$l,$p]): ?>
          <li><a href="<?= base_url($p) ?>" class="text-slate-500 transition-colors hover:text-slate-900"><?= $l ?></a></li>
          <?php endforeach; ?>
        </ul>
      </div>
      ...
      <!-- Tools -->
      <div>
        <h3 class="mb-4 font-bold text-xs uppercase tracking-widest" style="color:#0F172A">Tools & Calculators</h3>
        <ul class="space-y-2.5 text-sm">
          <?php foreach ([['EV Finder Quiz','find-my-ev'],['Subsidy Calculator','subsidy-calculator'],['Savings Calculator','tco-calculator'],['Charging Cost','charging-cost'],['Trip Range Check','can-i-make-it'],['Resale Estimator','resale-estimator'],['Fleet Calculator','fleet-calculator'],['EMI Calculator','ev-emi-calculator']] as [$l,$p]): ?>
          <li><a href="<?= base_url($p) ?>" class="text-slate-500 transition-colors hover:text-slate-900"><?= $l ?></a></li>
          <?php endforeach; ?>
        </ul>
      </div>
      ...
      <!-- Company -->
      <div>
        <h3 class="mb-4 font-bold text-xs uppercase tracking-widest" style="color:#0F172A">Company</h3>
        <ul class="space-y-2.5 text-sm">
          <?php foreach ([['Home Charger Guide','home-charger-guide'],['EV Glossary','ev-glossary'],['Charging Stations','charging-stations'],['News & Reviews','blog'],['Used EVs','used-ev'],['For Dealers','for-dealers'],['For Brands','for-brands'],['About Us','about'],['Contact','contact']] as [$l,$p]): ?>
          <li><a href="<?= base_url($p) ?>" class="text-slate-500 transition-colors hover:text-slate-900"><?= $l ?></a></li>
          <?php endforeach; ?>
        </ul>
      </div>
    ...
    <div class="mt-12 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-400" style="border-top:1px solid rgba(0,230,118,.12)">
      <p>&copy; <?= date('Y') ?> Charj.in — India's EV Decision Engine. Made with ⚡ in India.</p>
      <nav class="flex gap-6" aria-label="Legal links">
        <?php foreach ([['Privacy Policy','privacy-policy'],['Terms of Use','terms-of-service'],['Disclaimer','disclaimer'],['Sitemap','sitemap.xml']] as [$l,$p]): ?>
        <a href="<?= base_url($p) ?>" class="hover:text-slate-700 transition-colors"><?= $l ?></a>
        <?php endforeach; ?>
      </nav>
    </div>
    <p class="mt-4 text-xs text-slate-400 leading-relaxed max-w-4xl"><strong class="text-slate-500">Prices are indicative.</strong> Vehicle prices, range figures, specifications and availability are subject to change. Always verify with the official dealer or manufacturer. Charj.in is an independent platform not affiliated with any EV brand unless stated.</p>
  </div>
</footer>
...
<script>
(function(){
  /* 1. Scroll-reveal for .sr variants */
  ...
  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded',function(){ observeSR(); initCards(); initRipple(); });
  } else { observeSR(); initCards(); initRipple(); }
})();
</script>
...
(function(){
  ...
  window.charjToggleWishlist = function(slug, name){
    var list = load(WK);
    var idx  = list.findIndex(function(x){ return x.slug === slug; });
    if(idx >= 0){
      list.splice(idx,1);
...
<div id="charj-compare-bar"
     ...
     style="background:linear-gradient(135deg,#022C22 0%,#0A3D2B 60%,#0D4A2A 100%);border-top:1px solid rgba(0,230,118,.22);box-shadow:0 -6px 32px rgba(0,0,0,.35)">

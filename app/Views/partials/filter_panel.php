...
<div class="space-y-6">
  <div>
    ...
    <label class="flex items-center gap-2.5 text-sm py-1.5 cursor-pointer text-slate-400 hover:text-cyan-300 transition-colors">
      <input type="radio" name="category" value="<?= esc($cat['slug']) ?>"
        <?= $activeCategory === $cat['slug'] ? 'checked' : '' ?> class="accent-teal-500">
      <?= esc($cat['name']) ?>
    </label>
    <?php endforeach; ?>
  </div>
  <div>
    ...
    <label class="flex items-center gap-2.5 text-sm py-1.5 cursor-pointer text-slate-400 hover:text-cyan-300 transition-colors">
      <input type="checkbox" name="brand[]" value="<?= esc($brand['slug']) ?>"
        <?= in_array($brand['slug'], $activeBrands) ? 'checked' : '' ?> class="accent-teal-500">
      <?= esc($brand['name']) ?>
    </label>
    <?php endforeach; ?>
  </div>

...
<!-- Dark hero -->
<section style="background:linear-gradient(135deg,#0f172a,#1a3a55,#0f2a1f)" class="text-white pt-28 pb-16 px-4 relative overflow-hidden">
  ...
  <div class="relative max-w-3xl mx-auto text-center">
    ...
    <h1 class="text-4xl sm:text-5xl font-black leading-tight mb-3">Can Your EV Make<br>the Trip?</h1>
    <p class="text-slate-400 text-base">Enter your route distance — we'll tell you if you'll make it</p>
  </div>
</section>
    ...
    >
      ...
      <div>
        ...
        <div class="grid grid-cols-3 gap-2">
          <label :class="style==='city' ? 'ring-2 ring-green-500 bg-green-50' : 'ring-1 ring-slate-200 bg-slate-50 hover:bg-slate-100'" class="flex flex-col items-center gap-1 p-3 rounded-2xl cursor-pointer transition-all duration-200">
          ...
          <label :class="style==='highway' ? 'ring-2 ring-green-500 bg-green-50' : 'ring-1 ring-slate-200 bg-slate-50 hover:bg-slate-100'" class="flex flex-col items-center gap-1 p-3 rounded-2xl cursor-pointer transition-all duration-200">
          ...
          <label :class="style==='mixed' ? 'ring-2 ring-green-500 bg-green-50' : 'ring-1 ring-slate-200 bg-slate-50 hover:bg-slate-100'" class="flex flex-col items-center gap-1 p-3 rounded-2xl cursor-pointer transition-all duration-200">
      ...
      <!-- Result -->
      <div x-show="hasResult" x-transition:enter="transition-all duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-4">

        <!-- Big YES/NO -->
        <div class="text-center">
          <div
            :class="canMakeIt ? 'bg-green-50 ring-4 ring-green-100' : 'bg-amber-50 ring-4 ring-amber-100'"
            class="inline-flex flex-col items-center justify-center w-32 h-32 rounded-full"
          >
            <div :class="canMakeIt ? 'text-green-700' : 'text-amber-600'" class="text-4xl font-extrabold" x-text="canMakeIt ? 'YES' : 'NO'"></div>
            <div :class="canMakeIt ? 'text-green-600' : 'text-amber-500'" class="text-xs font-semibold" x-text="canMakeIt ? 'you can!' : 'charge needed'"></div>
          </div>
        </div>
        ...
        <div x-show="!canMakeIt" class="bg-amber-50 rounded-2xl p-4 text-center ring-1 ring-amber-200">
          ...
          <p class="text-xs text-amber-600 mt-1">Charge for ~45 mins at a fast charger to continue</p>
        </div>

        <!-- Remainder / Shortfall -->
        <div class="grid grid-cols-2 gap-3">
          <div x-show="canMakeIt" class="bg-slate-50 rounded-2xl p-3 text-center">
            <div class="text-xs text-slate-500 mb-1">Remaining range after trip</div>
            <div class="text-lg font-bold text-slate-800" x-text="remainder + 'km'"></div>
          </div>
          <div x-show="!canMakeIt" class="bg-amber-50 rounded-2xl p-3 text-center ring-1 ring-amber-100">
            <div class="text-xs text-amber-600 mb-1">Shortfall</div>
            <div class="text-lg font-bold text-amber-700" x-text="shortfall + 'km'"></div>
          </div>
          <div class="bg-slate-50 rounded-2xl p-3 text-center">
            <div class="text-xs text-slate-500 mb-1">Real-world efficiency</div>
            <div class="text-lg font-bold text-slate-800" x-text="factorPct + '%'"></div>
          </div>
        </div>
      ...
      <div x-show="!hasResult" class="text-center py-6 text-slate-300">
        <svg class="w-12 h-12 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"/></svg>
        <p class="text-sm">Enter a distance above to see your result</p>
      </div>

      <!-- CTA -->
      <a href="<?= base_url('find-my-ev') ?>" class="flex items-center justify-center gap-2 w-full py-3 rounded-2xl bg-slate-900 text-white text-sm font-semibold hover:bg-slate-700 transition-colors duration-200">

...
function cmpRow(string $label, array $vv, string $field, string $mode = 'text', string $unit = '', bool $formatPrice = false): string {
    $count   = count($vv);
    ...
    foreach ($vals as $i => $raw) {
        ...
        $html .= $isW
            ? '<span style="color:#00963C;font-weight:800;font-size:.88rem">' . $display . '</span><span class="ml-1.5 text-[10px] px-1.5 py-0.5 rounded-full font-black align-middle best-badge best-pulse" style="background:#00E676;color:#022C22">BEST</span>'
            : '<span style="color:#0F172A;font-weight:600">' . $display . '</span>';
        $html .= '</div>';
    }
    return $html . '</div>';
}
...
<!-- HERO HEADER -->
<div class="relative overflow-hidden py-14 px-4 sm:px-6" style="background:#F7FFFE">
  ...
  <div class="max-w-7xl mx-auto relative" style="z-index:1">
    ...
    <!-- Breadcrumb -->
    <nav class="text-xs mb-6 flex items-center gap-1.5">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
      <div>
        ...
        <p class="text-base leading-relaxed max-w-xl anim-hero-sub" style="color:#64748B;font-weight:500">Side-by-side specs, real-world range, monthly costs &amp; more — the most detailed EV comparison.</p>
      </div>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6 cmp-page-bg" x-data="compareApp()" x-init="init()">
  ...
  <!-- ═══ COMPARISON TABLE ═══ -->
  <div id="compare-table" class="rounded-2xl overflow-hidden" style="background:#FFFFFF;border:1px solid rgba(0,168,150,.18);box-shadow:0 8px 40px rgba(0,168,150,.08),0 2px 12px rgba(0,0,0,.06)">
    <div class="cmp-table">
      ...
      <?php foreach ([['Starting Price','starting_price','low','',true],['Max Price (Top Trim)','max_price','low','',true],['Ex-showroom Delhi','ex_showroom_delhi','low','',true]] as [$l,$f,$m,$u,$fp]): echo cmpRow($l,$vehicles,$f,$m,$u,$fp); endforeach; ?>

      <?= cmpSection('📏 Range', $count) ?>
      <?php foreach ([['Claimed Range (ARAI)','claimed_range','high','km'],['Real-World Range','real_world_range','high','km'],['City Range (est.)','city_range','high','km'],['Highway Range (est.)','highway_range','high','km']] as [$l,$f,$m,$u]): echo cmpRow($l,$vehicles,$f,$m,$u); endforeach; ?>

      <?= cmpSection('🔋 Battery', $count) ?>
      <?php foreach ([['Battery Capacity','battery_capacity','high','kWh'],['Battery Type','battery_type','text',''],['Battery Warranty','battery_warranty','text',''],['Charging Connector','connector_type','text','']] as [$l,$f,$m,$u]): echo cmpRow($l,$vehicles,$f,$m,$u); endforeach; ?>
  ...
  <!-- ═══ WINNER CARD ═══ -->
  <?php if ($bestVeh): ?>
  <div class="rounded-2xl p-6 md:p-8 anim-winner" style="background:linear-gradient(135deg,#040C1E,#071830,#0A1E3A);border:1px solid rgba(0,168,150,.25);box-shadow:0 8px 40px rgba(0,0,0,.3),0 0 0 1px rgba(0,168,150,.1)">
    <div class="flex flex-col md:flex-row items-start md:items-center gap-6">
      <div class="flex-1">
        <div class="flex flex-wrap items-center gap-2 mb-3">
          <span class="text-2xl">🏆</span>
          <span class="text-[11px] font-extrabold px-3 py-1 rounded-full uppercase tracking-wider" style="background:#F59E0B;color:#040C1E">Our Recommendation</span>
        </div>
        <h3 class="text-2xl md:text-3xl font-black mb-2 leading-tight text-white">
          Based on this comparison, <span style="color:#1AFFCC"><?= esc($bestVeh['name']) ?></span> wins overall
        </h3>
        <p class="text-sm leading-relaxed mb-4 font-medium" style="color:#94A3B8">
          Leads on <strong style="color:#fff"><?= $winCounts[$bestIdx] ?></strong> out of <strong style="color:#fff"><?= count($metricFields) + 1 ?></strong> key metrics including range, value, performance and monthly cost.
        </p>
        <div class="flex flex-wrap gap-2">
          <?php foreach ($vehicles as $vi => $vv): ?>
          <div class="flex items-center gap-2 px-3 py-1.5 rounded-xl" style="background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.1)">
            <span class="text-sm font-bold text-white"><?= esc($vv['name']) ?></span>
      ...
      <div class="flex flex-col gap-3 w-full md:w-auto md:min-w-[220px] flex-shrink-0">
        ...
        <a href="<?= base_url('find-my-ev') ?>"
           class="flex items-center justify-center gap-2 font-semibold px-6 py-3 rounded-xl text-white transition-all"
